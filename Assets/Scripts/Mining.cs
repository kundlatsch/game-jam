...
public class Mining : MonoBehaviour {
	...
	void Update() {
		if(endGame && SceneManager.GetActiveScene().name == "InGame"){		
	    ...
	    if(blackHoleAnim) {
	    	...
	    	if (blackHoleAnimF) {
	    		...
	    		if(GameObject.Find("Planet4").transform.position == blackHole.transform.position) {
	    ...
	    //End Game 2
	    if(endGame && SceneManager.GetActiveScene().name == "InGame2"){
	    	Debug.Log("OK");
	    	if(GlobalData.main_planet_id != 4){
	    		SceneManager.LoadScene("EndGame3", LoadSceneMode.Single);
	    	}else{
	    		SceneManager.LoadScene("EndGame4", LoadSceneMode.Single);
	    	}
	    }
	...
	void MineCycle() {
		...
		else if((GlobalData.main_planet_id == 4 && SceneManager.GetActiveScene().name == "InGame") ||
		 (GlobalData.main_planet_id == 0 && SceneManager.GetActiveScene().name == "InGame2") ) {
			sun.SetActive(false);
			sun2.SetActive(false);
			sun3.SetActive(true);
			endGame = true;
